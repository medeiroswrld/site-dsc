...
// inline: the "use client" directive the App Router requires, and ScrollTrigger
// loaded on demand instead of at module scope.
...
// LOCAL CHANGE. Upstream imports and registers ScrollTrigger at module scope,
// which puts ~30 kB of plugin on the critical path of whatever renders this —
// here, the page's H1. Only the "scroll" trigger needs it, so it is fetched
// then and only then.
...
const FoldText = ({
  ...
  useEffect(() => {
    ...
    if (trigger === 'hover') {
      ...
      // LOCAL CHANGE: plugin arrives asynchronously, so guard against the
      // component unmounting while the chunk is still in flight.
