...
// From React Bits (JavaScript + CSS variant). Two local changes, both marked
// inline: the "use client" directive the App Router requires, and a
// `wheelNavigation` prop so the component can be dropped into a scrolling page
// without capturing the page's scroll.
...
const DepthCarousel = ({
  ...
  showIndicators = true,
  onChange,
  // LOCAL ADDITION. Upstream always calls preventDefault on wheel, which traps
  // the page scroll whenever the pointer is over the carousel. Inside a long
  // marketing page that reads as a broken page, so the host decides.
  wheelNavigation = true,
  // LOCAL ADDITION. Upstream cards only re-focus on click; this lets a click on
  // the already-focused card do something useful, like opening the post.
  onItemActivate,
  className = ''
}) => {
  ...
  useEffect(() => {
    if (!wheelNavigation) return undefined; // LOCAL: opt out of scroll capture
  ...
  const onCardClick = useCallback(
    index => {
      ...
      // LOCAL: a click on the card already in front means "open this one".
      if (index === focusRef.current && onItemActivate) {
        onItemActivate(index, data[index]);
        return;
      }
      setFocus(index, true);
    },
    [setFocus, onItemActivate, data]
  );
